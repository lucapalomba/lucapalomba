...
- **Page front matter**: also supports `page_scripts` (list of additional JS paths loaded at the bottom), `hide_nav_scripts` (disables `navigation.js`/`hamburger.js`) and `body_class` (`technologies-page` is what `techProgress.js` looks for; `not-found-page` suppresses only the navigation hint — the 404 keeps full chrome, navbar and footer included). Client-side scripts that need to know which page they are on read `document.body.dataset.page` (the `i18n_key`), not the URL.
...
- `_includes/navbar.html` — sticky navbar with the `<LP />` brand, server-rendered links (active state via `page.i18n_key`), the availability badge with a single-cycle ping, the language switcher (from `page.alt_url`, rendered in both the desktop row and the mobile drawer), the decorative avatar glyph, hamburger + mobile drawer, "Curriculum" CTA (LinkedIn link). At 769–1023px the numbered row and CTA move into the drawer, since the export's five-item row is seven items here. Contains the drawer toggle logic. The typewriter sound toggle and its audio were removed with the revamp (D6), so `js/soundMute.js` and `sounds/` are gone.
...
- `_includes/transition-overlay.html` — markup for the full-screen overlay used for the short opacity page transition (D5).
...
- `index.html` — home: `layout: home`, `i18n_key: index`. Hero (intro, name, animated title, subtitle, bio, CTA) plus a `#hero-animation` JSON blob carrying the typewriter steps. Loads `titleAnimation.js` via `page_scripts`.
...
- `technologies.html` — technologies grid with progress bars (`.tech-progress-fill` with `data-progress`) animated by `techProgress.js`. The editorial order, percentage, tone and category per row live in the layout as a `key:percentage:tone:category:rise` table; the domain/vector/status copy is keyed in `technologies.meta` in both dictionaries.
...
- `transitions.js` — `PageTransition` class: intercepts clicks on internal links (excluding `target="_blank"` and skip-link), shows the overlay, waits for the token duration (~230ms) then navigates. Exit animation handled via the `active` class.
...
- `techProgress.js` — animates `.tech-progress-fill` widths on scroll and drives the technologies category filter (live count, `aria-pressed`, empty state), refilling and revealing only the cards the filter brings back (never pre-empting the scroll reveals on the initial pass).
- `backdrop.js` — moves the ambient backdrop's cursor-light layer with a single compositor `transform`, rAF-coalesced. Bails out under `prefers-reduced-motion` and on coarse-pointer (touch) devices, where there is no cursor to follow.
