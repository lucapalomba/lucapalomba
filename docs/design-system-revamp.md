...
- **D14 — Radii: token block or prose?** `DESIGN.md` states radii twice and the two disagree: its `rounded:` prose block says 4/8/12px, while the `tailwind.config` embedded in every mockup says DEFAULT 2px / lg 4px / xl 8px. `styles/tokens.css` takes the config values, because that is what the screens actually render. *Recommendation:* treat the config as canonical and the prose as stale (the same call D1 made for surfaces), and keep `--radius-sm` 2px / `--radius` 2px / `--radius-md` 4px / `--radius-lg` 4px / `--radius-xl` 8px. Do not "fix" them back to the prose. Blocks nothing; recorded so the next reader does not revert it.

**Refused motion layer (D13, enumerated).** The plan's D13 named only the two canvas backgrounds and the particles, but the diff that implemented it also refused thirteen distinct infinite effects from the `_unified_motion_text_fx` export. Listed here so the gap reads as a choice rather than an oversight: `text-shimmer` (hero claim, page 01), `pulse-glow` (accent surfaces, 01), `cyan-shimmer` (section headings, 02), `scanlineGlow` (timeline, 02), `techGradientShimmer` (card headings, 03), `shimmerLoop` (progress bars, 03), `subtlePulse` (status marks, 03), `shimmerTitle` (page title, 04), `radarRing` ×2 (contact radar, 04), `eqBar` ×4 (equaliser, 04), plus the Tailwind `animate-pulse` / `animate-shimmer` / `animate-bar` wrappers across 01–04. The port deliberately has a calmer ambient layer than the export: entrance reveals are faithful (including the stagger offsets), the ambient loops are not. Two of the repo's own infinite loops were also dropped under the same rule — the technologies chevrons and the `reduced-motion.css` comment that claimed to catch "every remaining infinite CSS animation" now names the one loop that is actually left (the hero typing indicator, which only runs between typewriter steps).
