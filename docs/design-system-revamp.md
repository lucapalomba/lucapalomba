...
| Bilingual content (EN + IT, 43 keys) | English only — no Italian reference for any of the new copy | R14 |
| Console easter egg (both locales) | Nothing | Keep |
...
| Language switcher | Nothing — and the site has none either: `switchLanguage()` exists in `js/i18n.js` but nothing calls it, so the language is browser-detected only | Not a gap. Note that `AGENTS.md` documents a manual switch that does not exist |

### The service worker is the one with teeth

`sw.js` keeps a **hand-written `PRECACHE_URLS` list naming every stylesheet, every script and the font file**:

```text
'./styles/main.css', … './js/hamburger.js', './js/soundMute.js',
'./js/particles.min.js',
'./fonts/Roboto_Mono/static/RobotoMono-Variable.woff2'
```

The revamp touches nearly all of it: R01 adds `styles/tokens.css`, R02 replaces the font path and deletes the Roboto Mono files, R19 adds motion CSS, and R06/R08 may delete `hamburger.js`, `soundMute.js` and `particles.min.js`. Each of those is an edit to that list.
...
- [ ] **R08 — Chrome effects audit** *([D4](#decisions-needed)–[D6](#decisions-needed))* — Apply the decisions on particles.js, the transition overlay and the typewriter/sound, and align `theme-color`, `manifest.webmanifest`'s `theme_color` / `background_color` (currently `#6e48aa` / `#0a0a0f`) and the favicon treatment with the new canvas. Delete the CSS and JS of whatever is dropped, and drop its entries from `sw.js`'s `PRECACHE_URLS` in the same PR (R20) — `js/particles.min.js` and `js/soundMute.js` are both precached today. *Done when:* no removed feature leaves dead files or a stale precache entry, and reduced-motion still disables what remains animated.
...
- [ ] **R14 — i18n for the new content** — Add every new string (pillars, principles, telemetry labels, metric rows, spectrum legend, statuses, CTAs, alt text) to both `translations/en.json` and `translations/it.json`, and extend `js/i18n.js` to render the new repeating structures the way it already renders `experiences.jobs`. The export offers no Italian reference for any of it, so this is authoring, not translation. *Done when:* `node scripts/check-i18n.js` passes, no raw key is visible in either language, and no existing key was renamed.
...
- **PR #127 (`feat/issue-82-server-side-i18n`)** — server-side i18n for SEO. It rewrites how `data-i18n` content is emitted, which is exactly the surface R14 extends. It is the last one still open; land it before R09–R13, or rebase those onto it, otherwise the two collide in `js/i18n.js` and every page's markup.
